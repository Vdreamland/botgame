...
def draw_status_table(bots_state: dict, num_bots: int):
    ...
    print("| Agent      | Redeem  | Weekly Reward | Target      | Room      | Status              |")
    print("-----------------------------------------------------------------------------------")
    for name, state in bots_state.items():
        agent = f"{name:<10}"
        redeem = f"{state.get('redeem', '-'):<7}"
        weekly = f"{state.get('weekly', '-'):<13}"
        target = f"{state.get('target', '-'):<11}"
        room = f"{state.get('room', '-'):<9}"
        status = f"{state.get('status', '-'):<19}"
        print(f"| {agent} | {redeem} | {weekly} | {target} | {room} | {status} |")
